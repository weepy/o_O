...
o_O.context = function(fn) {
	var context = $(this).data("o_O:context")
	return function(e) { 
		fn.call(context) //$(this).data("o_O:context")
	}
}

// o_O.event = function(fn) {
// 	return function(e) {
// 		fn() 
// 	}
// }
...
o_O.property = function(value) {
  ...
  prop.incr = function(x) {
		return prop(prop() + (x||1))
	}
		
	prop.change = function(fn) {
		fn 
			? prop.on('set', fn)   // listen for a change
			: prop.emit('set')     // trigger a change
	}
  return prop
}
...
o_O.bindings.foreach = function(collection, $el) {
  collection.render($el)
}

var Collection = function(array) {
  this.objects = {}
	this.length = o_O.property(0)
  o_O.eventize(this) 
  if(array) {
...
fn.count = function(fn) {
  var count = 0;
	this.length(); // depends on this!
